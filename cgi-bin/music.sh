...
# playlist, song, action [-/open/load/play/stop/next/back/add/loop/shuffle]
declare -A get_info
...
if [ -z "${get_info[playlist]}" ] && [ -z "${get_info[song]}" ] && [ -z "${get_info[action]}" ]; then
	...
	case "${get_info[action]}" in
		...
		"back" )
			...
			;;
		"add")
			echo "a ${get_info[playlist]} ${get_info[song]}" > "$music_manager_solicitations"
			;;
		"loop")
			echo "w" > "$music_manager_solicitations"
			;;
		"shuffle")
			echo "z ${get_info[playlist]}" > "$music_manager_solicitations"
			;;
		*)
			echo "Status: 400"
			echo "content-type: text/plain"
